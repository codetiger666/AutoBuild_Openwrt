...
Diy_Part1_Base() {
	...
	if [[ "${ChangeLanIp}" = "true" ]];then
		sed -i 's/ipad=${ipaddr:-"192.168.1.1"}/ipad=${ipaddr:-"10.10.1.1"}/' package/base-files/files/bin/config_generate
		sed -i 's/${ipaddr:-"192.168.$((addr_offset++)).1"}/${ipaddr:-"10.10.$((addr_offset++)).1"}/' package/base-files/files/bin/config_generate
	fi
}
